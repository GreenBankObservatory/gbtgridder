...
def get_cube_info(cubeFile, verbose=4):
    """Return a dictionary containing the information on the first 2 axes of
    the indicated cube FITS file.
    ...
    try:
        ...
        if cdelt1 > 0.0 or cdelt2 < 0.0:
            if verbose > 1:
                print(
                    "1st and/or 2nd axes of %s are inverted from expected direction"
                    % cubeFile
                )
            return result
        ...
        if (
            result["radesys"] is None
            and result["equinox"] is not None
            and result["xtype"] == "RA"
        ):
            result["radesys"] = "FK5"
