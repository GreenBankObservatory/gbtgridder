...
def grid_otf(spec_array, spec_size, nx, ny, glong, glat, pix_scale, refXsky, centerYsky, weight=None, beam_fwhm=None, kern="gaussbessel", _D=100, gauss_fwhm=None, verbose=4):
    ...
    final_fwhm = np.sqrt(beam_fwhm ** 2.0 + gauss_fwhm ** 2.0) #verison0.5 equivialent of scale_fwhm*beam_fwhm
    ...
    remove = (np.abs(glong_diff) > support_distance) + np.isnan(glong_diff) #testing is nan and returns true or false # removing anything that is greater than the support dist
    glong_diff[remove] = np.inf # this makes the distance of the removed value infinity
    glong_diff = sparse.COO(glong_diff, fill_value=np.inf) # makes sparse matrix of the difference - the fill value (values with 0) is infinity so the dist is too great
